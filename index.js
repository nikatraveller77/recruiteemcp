...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Створити нового кандидата в Recruitee',
    inputSchema: { type: 'object', properties: { name: { type: 'string', description: 'Імʼя кандидата' }, email: { type: 'string', description: 'Email' }, phone: { type: 'string', description: 'Телефон' }, offer_id: { type: 'number', description: 'ID вакансії (необовʼязково)' } }, required: ['name'] }
  }
];
...
${notes || '  немає'}`;
    ...
    case 'get_candidates_by_vacancy': {
      let list = [];
      const data1 = await recruitee(`/candidates?offer_id=${args.offer_id}&limit=100`);
      list = data1.candidates || [];
      if (!list.length) {
        const data2 = await recruitee(`/offers/${args.offer_id}/candidates?limit=100`);
        list = data2.candidates || data2.applications || [];
      }
    ...
    case 'create_candidate': {
      const payload = { candidate: { name: args.name } };
      if (args.email) payload.candidate.emails = [args.email];
      if (args.phone) payload.candidate.phones = [args.phone];
      const data = await recruitee('/candidates', 'POST', payload);
      console.log('[create_candidate] response:', JSON.stringify(data).slice(0, 300));
      const c = data.candidate || data;
      if (!c || !c.id) return `Помилка: ${JSON.stringify(data).slice(0, 200)}`;
      // Assign to vacancy if offer_id provided
      if (args.offer_id && c.id) {
        await recruitee(`/offers/${args.offer_id}/candidates`, 'POST', { candidate: { id: c.id } });
      }
