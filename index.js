...
${notes || '  немає'}`;
    ...
    case 'create_candidate': {
      ...
      if (args.email) payload.candidate.email = args.email;
      if (args.phone) payload.candidate.phone = args.phone;
      if (args.offer_id) payload.candidate.offer_id = args.offer_id;
      const data = await recruitee('/candidates', 'POST', payload);
      console.log('[create_candidate] response:', JSON.stringify(data).slice(0, 200));
      const c = data.candidate || data;
